...
public class MainPanel extends javax.swing.JPanel implements MouseListener, MouseMotionListener {
  ...
  private void initComponents() {
      ...
      .addGroup(layout.createSequentialGroup()
        ...
        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
          ...
          .addGroup(layout.createSequentialGroup()
            ...
            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
            .addComponent(desc, javax.swing.GroupLayout.DEFAULT_SIZE, 251, Short.MAX_VALUE)
            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
            .addComponent(cancel)
            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
            .addComponent(choose)))
    ...
    layout.setVerticalGroup(
      ...
      .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
        .addContainerGap()
        .addComponent(map, javax.swing.GroupLayout.DEFAULT_SIZE, 501, Short.MAX_VALUE)
  ...
  private void initMap() {
    imgorg = new JFImage();
    imgorg.load(getClass().getResourceAsStream("/gmap-300.png"));
    img = new JFImage();
  ...
  private void ubuntu_initMirrors() {
    ...
    StringBuffer file;
    int i1,i2;
    //load saved system mirrors = /etc/jrepo-ubuntu.list
    ArrayList<Mirror> systemList = new ArrayList<Mirror>();
    try {
      FileInputStream fis = new FileInputStream("/etc/jrepo-ubuntu.list");
    ...
    //load saved personal mirrors = ~/.jrepo-ubuntu.list
    ArrayList<Mirror> userList = new ArrayList<Mirror>();
    try {
      FileInputStream fis = new FileInputStream(JF.getUserPath() + "/.jrepo-ubuntu.list");
      byte buf[] = JF.readAll(fis);
      fis.close();
      String lns[] = new String(buf, "UTF-8").split("\n");
      for(int i=0;i<lns.length;i++) {
    ...
    //save any new mirrors to ~/.jrepo-ubuntu.list
    try {
      FileOutputStream fos = new FileOutputStream(JF.getUserPath() + "/.jrepo-ubuntu.list", true);
      int cnt = 0;
      for(int j=0;j<mirrors.size();j++) {
        m = mirrors.get(j);
        if (!m.isNew) continue;
      ...
      fos.close();
      JFLog.log("Wrote " + cnt + " new entries to ~/.jrepo-ubuntu.list");
    } catch (Exception e) {
      JFLog.log(e);
    }
  }
  ...
  private void fedora_initMirrors() {
    ...
    StringBuffer file;
    int i1,i2;
    //load saved system mirrors = /etc/jrepo-fedora.list
    ArrayList<Mirror> systemList = new ArrayList<Mirror>();
    try {
      FileInputStream fis = new FileInputStream("/etc/jrepo-fedora.list");
      byte buf[] = JF.readAll(fis);
      fis.close();
      String lns[] = new String(buf, "UTF-8").split("\n");
      for(int i=0;i<lns.length;i++) {
    ...
    //load saved personal mirrors = ~/.jrepo.list
    ArrayList<Mirror> userList = new ArrayList<Mirror>();
    try {
      FileInputStream fis = new FileInputStream(JF.getUserPath() + "/.jrepo-fedora.list");
      byte buf[] = JF.readAll(fis);
      fis.close();
      String lns[] = new String(buf, "UTF-8").split("\n");
      for(int i=0;i<lns.length;i++) {
    ...
    //save any new mirrors to ~/.jrepo.list
    try {
      FileOutputStream fos = new FileOutputStream(JF.getUserPath() + "/.jrepo-fedora.list", true);
      int cnt = 0;
      for(int j=0;j<mirrors.size();j++) {
        m = mirrors.get(j);
        if (!m.isNew) continue;
      ...
      fos.close();
      JFLog.log("Wrote " + cnt + " new entries to ~/.jrepo-fedora.list");
    } catch (Exception e) {
      JFLog.log(e);
    }
  }
  ...
  private void calcMapCoords() {
    ...
    double ix = img.getWidth();
    double iy = img.getHeight();
    double xOffset = ix * -0.024390;  //offset (approx where meridian is relative to center)
    double yOffset = iy * 0.18855;  //offset (approx where equator is relative to center)
    double xScale = 1.04;
    double yScale = 4.35;
    for(int a=0;a<mirrors.size();a++) {
      Mirror m = mirrors.get(a);
      m.x = (int)((((m.lng / xScale) + 180.0) / 360.0 * ix) + xOffset);
      if (m.x < 0) m.x += ix;
      double lat_rad = Math.toRadians(m.lat);
      m.y = (int) (((Math.log((1.0 + Math.sin(lat_rad)) / Math.cos(lat_rad)) * iy) * -1.0 / yScale) + (iy/2 + yOffset));
      for(int b=0;b<a;b++) {
        Mirror mb = mirrors.get(b);
        int d = (int)Math.sqrt(Math.pow(m.x - mb.x, 2) + Math.pow(m.y - mb.y, 2));
        if (d < 3) {
          m.x -= 2;
          m.y -= 2;
          b = -1;  //restart
  ...
  private void updateDesc() {
    ...
    desc.setText(m.country + ":" + m.name + ":" + m.hostname + ":" + m.bandwidth);
  }
