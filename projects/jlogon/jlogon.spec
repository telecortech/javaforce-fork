...
if [ $1 -eq 1 ] ; then
  ...
  /usr/bin/systemctl preset jlogon.service >/dev/null 2>&1 || :
fi
...
if [ $1 -eq 0 ] ; then
  ...
  /usr/bin/systemctl --no-reload disable jlogon.service > /dev/null 2>&1 || :
  /usr/bin/systemctl stop jlogon.service > /dev/null 2>&1 || :
fi
