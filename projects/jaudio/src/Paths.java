...
public class Paths {
  ...
  public static String getNewProjectPath() {
    if (JF.isWindows()) {
      return System.getenv("TEMP") + "/jaudio-" + JF.getCurrentUser() + "/project-" + Math.abs(r.nextInt());
    } else {
      return "/var/tmp/jaudio-" + JF.getCurrentUser() + "/project-" + Math.abs(r.nextInt());
    }
  }

  public static String getClipboardPath() {
    if (JF.isWindows()) {
      return System.getenv("TEMP") + "/jaudio-" + JF.getCurrentUser() + "/clipboard";
    } else {
      return "/var/tmp/jaudio-" + JF.getCurrentUser() + "/clipboard";
    }
  }
