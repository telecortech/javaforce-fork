...
//      JFLog.log("paintHiRes:offset=" + project.offset + ":rateScale=" + rateScale + ":int=" + intRateScale);
...
//      JFLog.log("selectStop:" + pos + "," + selectStop);
      repaint();
    }
  ...
  public void record(int recordingRate) {
    selectTrack(true);
    this.rate = recordingRate;
    recording = true;
    new Thread() {
      public void run() {
        long length = 0;
        Sound.Input input = Sound.getInput(true);
        if (!input.start(1, rate, 16, 1024, null)) return;
  ...
  public void play() {
    ...
    new Thread() {
      public void run() {
        ...
        int numSamples;
        output.start(channels, rate, bits, bufferSize, null);
        while (playing) {
   ...
   * Assumes : channel is mono (primarily for recording from mic)
   */
...
//JFLog.log("addSamples(1):" + lastChunk.cid + "," + lastChunk.offset +"," + lastChunk.length);
        try {
          ...
          file.write(data);
          int allLength = fileLength - header.length + data.length;
          byte allSamples[] = new byte[allLength];
          file.seek(header.length);
          JF.readAll(file, allSamples, 0, fileLength - header.length);
          System.arraycopy(data, 0, allSamples, fileLength - header.length, data.length);
          file.close();
    ...
    try {
      ...
      do {
        ...
        for(int ch=0;ch<channels;ch++) {
          ...
          JF.readAll(fis, samples, 0, chunk.length * bytes);
          FileOutputStream fos = new FileOutputStream(project.path + "/" + tid + "/c" + cid + "-" + ch + ".dat");
          chunk.write(fos);
