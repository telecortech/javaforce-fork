...
//interface DialChain
  ...
  public boolean onInvite(CallDetailsPBX cd, SQL sql, boolean src) {
    ...
    if ((cd.user == null) || (cd.user.length() == 0)) return true;
    String ext = sql.select1value("SELECT ext FROM exts WHERE ext=" + sql.quote(cd.user));
    if (ext == null) return true;  //an extension is not dialing
    if (!api.isRegistered(cd.user)) return true;
    if (cd.user.equals(cd.dialed)) return true;  //voicemail may intercept next
    ...
    if ((cd.trunks == null) || (cd.trunks.length < 2)) return true;
