...
public class Service implements SIPServerInterface, PBXAPI {
  ...
  public boolean init() {
    ...
    //start service
    ss = new SIPServer();
    int port, rtpmin, rtpmax;
    try {
      port = Integer.valueOf(getCfg("port"));
...
//interface SIPServerInterface
  ...
  public void onInvite(CallDetailsServer cd, boolean src) {
    ...
    for(int a=0;a<dialChainList.size();a++) {
      ...
      if ((cd.pid == 0) || (cd.pid == chain.getPriority())) {
        if (!chain.onInvite((CallDetailsPBX)cd, sql, src)) {
          sql.uninit();
          return;
        }
      }
  ...
  public void onCancel(CallDetailsServer cd, boolean src) {
    ...
    for(int a=0;a<dialChainList.size();a++) {
      chain = dialChainList.get(a);
      if (cd.pid == chain.getPriority()) {
        if (!chain.onCancel((CallDetailsPBX)cd, sql, src)) break;
      }
    }
    sql.uninit();
  }
  public void onBye(CallDetailsServer cd, boolean src) {
    ...
    for(int a=0;a<dialChainList.size();a++) {
      chain = dialChainList.get(a);
      if (cd.pid == chain.getPriority()) {
        if (!chain.onBye((CallDetailsPBX)cd, sql, src)) break;
      }
    }
    sql.uninit();
  }
  public void onSuccess(CallDetailsServer cd, boolean src) {
    ...
    for(int a=0;a<dialChainList.size();a++) {
      chain = dialChainList.get(a);
      if (cd.pid == chain.getPriority()) {
        if (!chain.onSuccess((CallDetailsPBX)cd, sql, src)) break;
      }
    }
    sql.uninit();
  }
  public void onRinging(CallDetailsServer cd, boolean src) {
    ...
    for(int a=0;a<dialChainList.size();a++) {
      chain = dialChainList.get(a);
      if (cd.pid == chain.getPriority()) {
        if (!chain.onRinging((CallDetailsPBX)cd, sql, src)) break;
      }
    }
    sql.uninit();
  }
  public void onError(CallDetailsServer cd, int code, boolean src) {
    ...
    for(int a=0;a<dialChainList.size();a++) {
      chain = dialChainList.get(a);
      if (cd.pid == chain.getPriority()) {
        if (!chain.onError((CallDetailsPBX)cd, sql, code, src)) break;
      }
    }
    sql.uninit();
  }
  public void onTrying(CallDetailsServer cd, boolean src) {
    ...
    for(int a=0;a<dialChainList.size();a++) {
      chain = dialChainList.get(a);
      if (cd.pid == chain.getPriority()) {
        if (!chain.onTrying((CallDetailsPBX)cd, sql, src)) break;
      }
    }
    sql.uninit();
  }
  public void onFeature(CallDetailsServer cd, String cmd, String cmddata, boolean src) {
    ...
    for(int a=0;a<dialChainList.size();a++) {
      chain = dialChainList.get(a);
      if ((cd.pid == 0) || (cd.pid == chain.getPriority())) {
        if (!chain.onFeature((CallDetailsPBX)cd, sql, cmd, cmddata, src)) break;
      }
    }
    sql.uninit();
  }
  ...
  private void registerTrunks() {
    ...
    try {
      ...
      for(int trunk=0;trunk<trunks.length;trunk++) {
        String register = trunks[trunk];  //user : password @ domain[:port] [/ did]
