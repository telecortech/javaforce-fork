...
final class g729a_decoder implements g729a_constants
{
	...
	if (!bool) {
	    ...
	    g729a_common.g729a_lsp_interpolation
		(lp.q_quant_present_subframe_1, lp.q_quant_past_subframe_2,
		 lp.q_quant_present_subframe_2);
	    g729a_common.g729a_LSP2LP((var_g729a_decode_internal
				       .a_quant_subframe1),
	    ...
	    g729a_reconstruct.g729a_dec_gain(var_g729a_decode_internal,
					     speech);
	    g729a_reconstruct.g729a_reconstruct_speech
		(var_g729a_decode_internal, speech, filters,
	    ...
	    speech.past_quan_gc = var_g729a_decode_internal.gc_quan
		= 0.98F * speech.past_quan_gc;
		...
		g729a_common.g729a_generation_adaptive_vector
		    (var_g729a_decode_internal.v, speech.excitation, speech.T2,
		     0);
	...
	for (int i = 0; i < 10; i++)
	    fs_20_[i] = fs_21_[i] = fs_22_[i] = fs_23_[i]
		= (float) (i + 1) * f;
	for (int i = 0; i < 2; i++) {
		...
		fs_23_[i_26_ + i_24_]
		    = (1.0F - fs[i_26_ + i_25_]) * f * (float) (i_26_ + 1);
	}
	...
	g729a_preproc_filter var_g729a_preproc_filter = postproc;
	g729a_sinthesis_filter var_g729a_sinthesis_filter = sinthesis;
	g729a_sinthesis_filter var_g729a_sinthesis_filter_27_
	    = residual_filter;
	...
	for (int i = 0; i < 10; i++)
	    fs_29_[i] = fs_30_[i] = fs_31_[i] = fs_32_[i]
		= (float) (i + 1) * f;
	for (int i = 0; i < 2; i++) {
		...
		fs_32_[i_35_ + i_33_]
		    = (1.0F - fs[i_35_ + i_34_]) * f * (float) (i_35_ + 1);
	}
	filters.postproc = var_g729a_preproc_filter;
	filters.sinthesis = var_g729a_sinthesis_filter;
