...
//      System.out.println("move:" + file.getAbsolutePath() + " to " + pathTrash.getAbsolutePath());
  ...
  public void local_cut() {
    ccp_files = new ArrayList<String>();
    ccp_copy = false;
    FileEntry files[] = localBrowser.getSelected();
    localBrowser.setSelectedTransparent();
    for(int a=0;a<files.length;a++) {
      ccp_files.add(files[a].file);
    }
    //TODO : make icons 50% lighter(transparent)
  }

  //NOTE : cut/copy/paste is for SiteLocal only
  ...
  public void remote_cut() {
    ...
    ccp_copy = false;
    FileEntry files[] = remoteBrowser.getSelected();
    remoteBrowser.setSelectedTransparent();
    for(int a=0;a<files.length;a++) {
      ccp_files.add(files[a].file);
    }
    //TODO : make icons 50% lighter(transparent)
